...
function CareerCard({ career, probability, description, isTop, isSelected, onSelect }) {
  ...
  return (
    ...
    >
      {isTop && (
        <span className="badge-orange text-xs mb-3">Top Match</span>
      )}
      ...
      <div className="w-full bg-klenz-border rounded-full h-1.5 mb-4">
        <div className="bg-orange-gradient h-1.5 rounded-full transition-all" style={{ width: `${pct}%` }} />
      </div>
      <p className="text-klenz-muted text-sm mb-3 leading-relaxed">{description}</p>
      <div className="flex flex-wrap gap-1.5">
        {(TAGS[career] || []).map((tag) => (
          <span key={tag} className="text-xs bg-klenz-card text-klenz-muted px-2.5 py-1 rounded-full border border-klenz-border">
            {tag}
          </span>
        ))}
      </div>
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (analysis.career_matches?.[0]) {
        ...
        localStorage.setItem('selectedCareer', analysis.career_matches[0].career);
      }
    }
  }, []);
  ...
  if (matches.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        <h1 className="text-2xl font-bold mb-2">Career Matches</h1>
        <p className="text-klenz-muted text-sm mb-8 max-w-md">
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-klenz-muted">Ranked by probability based on your evidence</p>
        </div>
        {selectedCareer && (
          <Link to="/dashboard/roadmap" className="btn-orange text-sm self-start">
            View Roadmap →
          </Link>
        )}
      </div>
