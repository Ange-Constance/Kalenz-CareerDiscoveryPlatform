...
function CompetencyCard({ name, score, index }) {
  ...
  return (
    <div className="panel-elevated p-5 animate-slide-up" style={{ animationDelay: `${index * 0.08}s` }}>
...
export default function ProfilePage() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Analysis failed. Upload evidence first.');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <p className="text-klenz-muted text-sm mb-8 max-w-md">
          Run analysis on your uploaded evidence to generate your AI-powered talent profile.
        </p>
        {error && (
        ...
        <button onClick={runAnalysis} disabled={loading} className="btn-orange flex items-center gap-2">
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-klenz-muted">AI-generated narrative based on your evidence</p>
        </div>
      ...
      <div className="panel-elevated p-6 mb-8 border-klenz-orange/10 animate-fade-in">
        <h3 className="text-sm font-semibold text-klenz-orange mb-3">Your Career Narrative</h3>
        <p className="text-klenz-muted text-sm leading-relaxed">{analysis.narrative}</p>
      </div>

      <h3 className="text-sm font-semibold mb-4 text-klenz-muted uppercase tracking-wider">Competency Signals</h3>
